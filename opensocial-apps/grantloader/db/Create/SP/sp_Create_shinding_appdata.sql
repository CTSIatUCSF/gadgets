	...
	while @@fetch_status = 0 begin
		if(@currentPID != @pid) begin
			if  @currentPID != 0 begin
				...
				values(@personID, 'NIH Grants Awarded', 'nih_n', @grantCount, GetDate(), GetDate())
			end
		...
		--print @json
		select @cnt = count(*) from shindig_appdata where appId = 'NIH Grants Awarded' and userId = @personID and keyName = 'nih_n'
		if(@cnt = 0) begin
			insert shindig_appdata (userId, appId, keyName, value, createdDT, updatedDT)
			values(@personID, 'NIH Grants Awarded', 'nih_' + cast(@grantCount as varchar), @json, GetDate(), GetDate())
