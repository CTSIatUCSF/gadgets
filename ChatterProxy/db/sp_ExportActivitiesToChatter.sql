...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[UCSF.].[ExportActivitiesToChatter]') AND type in (N'P', N'PC')) BEGIN
	DROP PROCEDURE [ORNG.UCSF].[ExportActivitiesToChatter]
END
GO
...
-- =============================================
CREATE PROCEDURE [ORNG.UCSF].[ExportActivitiesToChatter] 
	@url nvarchar(256),
...
BEGIN
	...
	FROM [ORNG.UCSF].[ChatterActivity] 
	...
	BEGIN
		BEGIN TRY
			...
			exec [ORNG.UCSF].[CreateChatterActivity] @url, @username, @password, @token, @createdDT, @externalMessage, @employeeId, @actUrl, @actTitle, @actBody
			UPDATE [ORNG.UCSF].[ChatterActivity] SET chatterFlag = 'S', chatterAttempts = @attempts, updatedDT = GETDATE() WHERE activityLogId = @activityLogId
		END TRY
		BEGIN CATCH
			...
			IF @attempts > 0 BEGIN
				UPDATE [ORNG.UCSF].[ChatterActivity] SET chatterFlag = 'F', chatterAttempts = @attempts, updatedDT = GETDATE() WHERE activityLogId = @activityLogId
			END	
			ELSE BEGIN
				UPDATE [ORNG.UCSF].[ChatterActivity] SET chatterAttempts = @attempts, updatedDT = GETDATE() WHERE activityLogId = @activityLogId
			END
